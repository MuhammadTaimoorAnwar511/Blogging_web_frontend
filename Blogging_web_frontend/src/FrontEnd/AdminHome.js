    ...
    <div>
      ...
      <nav className="navbar">
        <ul>
          ...
          <li>
            <Link to="/adminpeople" className="notification-icon">
      ...
      <div className="search-div">
        ...
        />
        <button onClick={handleButtonClickblog}>Search</button>
      </div>*/}
      {/* Blog List */}
      <div className="blog-list">
        {userBlogs.map((blog) => (
          <div key={blog._id} className="blog-card1">
            {/* add block user button here */}
            <button className="block-button" onClick={() => toggleUserBlockStatus(blog.author._id)}>
              {blockedUsers[blog.author._id] ? 'Unblock User' : 'Block User'}
            </button>
                  {/* Enable/Disable Post Button */}
            <button className="block-post-button" onClick={() => togglePostBlockStatus(blog._id)}>
                {blockedpost[blog._id] ? 'Enable Post' : 'Disable Post'}
            </button>
